...
This program needs Boost and ZLib libraries to work with gzipped files in input and output.

If a common line couple is found, the program writes those lines in two corresponding output files. 
Also output is made on compressed text files.

COMMAND LINE USAGE:
	"./common_lines_to_files.x processed_pileup_1 processed_pileup_2 common_processed_pileup_1 common_processed_pileup_2"
	

Operatively it reads processed_pileup_2 and search it's lines in processed_pileup_1.

NOTE THAT:
For consistency the output of a run of this program with swapped arguments, file_1 with file_2, 
should be the same! It should find the same common lines, but the numbers of discarded and read lines may be different.
...
int main(int argc, char** argv){
	...
	if (argc != 5){
		cerr << "Usage: " << argv[0] << " processed_pileup_1 processed_pileup_2" 
		<< " common_lines_file_1 common_lines_file_2" << endl;
		return -1;
	}
	//filenames args
	...
	string common_1 = argv[3];
	string common_2 = argv[4];
	//*******************************************
	//          COMPRESSED INPUT
	//*******************************************
	...
	//*******************************************
	// Out for file 1
	ofstream out_file_1(common_1, ios_base::out | ios_base::binary);
	...
	// Out for file 2
	ofstream out_file_2(common_2, ios_base::out | ios_base::binary);
	...
	while (getline(file_2_instream, file_2_line)) { //until file_2 EOF
		...
		mut_freq_2 = compute_mut_freq(file_2_ATCG_data);
		...
		if ((file_1_chromosome_number == file_2_chromosome_number)
			...
			mut_freq_2 = compute_mut_freq(file_2_ATCG_data);
			mut_freq_1 = compute_mut_freq(file_1_ATCG_data);
